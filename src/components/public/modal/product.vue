<template>
  <!-- 商品列表 -->
  <div class="product-list-modal-part" v-if="isModal" @click.stop="closeModal">
                  ...
                  >
                    ...
                    <div class="related-content-wrap">
                      ...
                      <div class="all-select-part">
                        <!-- <input class="select-icon" v-model="checked1" @click="check($event)" type="checkbox" name="check" id=""> -->
                        <div
                          class="select-icon iconfont"
                          :class="{iconduihao: isAllCheck}"
                          @click="checkAll"
                        ></div>
                        <div class="name">全选</div>
                        <div class="group-name">
                          筛选出产品(
                          <span class="tnum">{{goodsList.length}}</span>)
                        </div>
                        <span class="collect-icon iconfont iconjiantou_liebiaozhankai-copy2"></span>
                      </div>
                      <div class="product-list-wrap">
                        ...
                        >
                          ...
                          <p v-if="pullLoading" class="loading-text">加载中...</p>
                    ...
                    <div class="bottom-result-wrap">
                      <div class="select-result">
                        共选中&nbsp;
                        <span class="cnum">{{currentList.length}}</span>&nbsp;个产品
                      </div>
...
export default {
  ...
  computed: {
    currentList() {
      ...
      let arr = [];
      if (this.setData.isDeleteGoods) {
        this.goodsList.forEach((ele) => {
          if (
            this.setData.chooseProductIds &&
            this.setData.chooseProductIds.indexOf(ele.id) > -1
          ) {
            ele.checked = true;
            arr.push(ele.id);
          } else {
        ...
        this.setData.isDeleteGoods = false;
      } else {
        this.goodsList.forEach((ele) => {
          if (ele.checked) {
            arr.push(ele.id);
    ...
    isModal() {
      let flag = this.$store.state.isProductModalShow;
      return this.$store.state.isProductModalShow;
    },
    isContent() {
  ...
  updated() {
    if (this.$store.state.isProductModalShow) {
  ...
  methods: {
    // 关闭弹窗
    closeModal() {
      this.$store.state.isProductModalShow = false;
    ...
    // 点击确定 添加
    sure() {
      this.setData.chooseProductList = [];
      ...
      this.goodsList.forEach((ele) => {
        if (this.currentList.indexOf(ele.id) > -1) {
          ele.checked = true;
          ele.image = ele.goods_imgs[0];
          this.setData.chooseProductList.push(ele);
        }
      });
      this.setData.chooseProductIds = this.currentList;
      this.setData.isGoodsFlag = true;
      this.closeModal();
    },
